...
async function handle(path: string, init?: RequestInit): Promise<Response> {
  ...
  if (path === '/api/products' && method === 'POST') {
    ...
    const input = await body(init); const product = { id: id(), name: String(input.name || '').trim(), sku: String(input.sku || '').trim(), category: String(input.category || '').trim(), stock: Number(input.stock), reorder: Number(input.reorder), price: Number(input.price), cost: Number(input.cost || 0), unit: String(input.unit || '').trim(), updated: now() }
