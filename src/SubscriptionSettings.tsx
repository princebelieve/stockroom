...
type Plan = { amount: number; currency: string; days: number; reminderDays: number; freeTrialDays: number }
...
export function SubscriptionSettings({ apiUrl, token, onAccess }: { apiUrl: string; token: string; onAccess: (access: SubscriptionAccess) => void }) {
  ...
  return <>
    <section className="panel full-panel subscription-panel"><div className="panel-heading"><div><h2>Subscription</h2><p>Manage your business plan and renew securely with Paystack.</p></div><AsyncButton className="text-button" busyLabel="Refreshing…" onClick={load}>Refresh</AsyncButton></div>
      ...
      {plan && <AsyncButton className="primary-button" busyLabel="Opening secure checkout…" onClick={startCheckout}>Renew with Paystack</AsyncButton>}
    </section>
