...
function App() {
  ...
  useEffect(() => {
    fetch('/api/settings').then((response) => response.ok ? response.json() as Promise<AppSettings & { ownerConfigured?: boolean; cloudConfigured?: boolean; existingBusiness?: boolean }> : Promise.reject()).then((settings) => {
      setRegistrationAvailable(!settings.ownerConfigured && !settings.cloudConfigured)
  ...
  if (!user) return registrationAvailable && registrationRequested ? <SetupScreen keyRegistration onCancel={() => { setRegistrationRequested(false); setAuthError('') }} onCreate={registerBusiness} error={authError} setError={setAuthError} /> : installerRequired ? <InstallerScreen onActivate={activateInstallation} message={installerMessage} onRegister={registrationAvailable ? () => setRegistrationRequested(true) : undefined} /> : setupRequired ? <SetupScreen onCreate={completeSetup} error={authError} setError={setAuthError} /> : <LoginScreen onLogin={login} error={authError} setError={setAuthError} onRegister={registrationAvailable ? () => setRegistrationRequested(true) : undefined} />
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {active === 'POS' && (posAccess.blocked || posAccess.status === 'grace') && <section className="panel full-panel" role="status"><h2>{posAccess.blocked ? 'POS access paused' : 'Subscription grace period'}</h2><p>{posAccess.reason}</p>{user.role === 'owner' && <button className="primary-button" onClick={() => setActive('Subscription')}>Manage subscription</button>} <AsyncButton className="text-button" busyLabel="Checking..." onClick={async () => setPosAccess(await readPosAccess(authToken, user.organizationId, true))}>Check access again</AsyncButton></section>}
