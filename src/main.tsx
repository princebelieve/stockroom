...
function App() {
  ...
  async function completeSale() {
    if (paymentMethod === 'external-pos' && !manualTerminalAllowed) { window.alert('Terminal integration is unavailable and manual confirmation is disabled. Update the terminal profile in Admin Settings.'); return }
    if (paymentMethod === 'external-pos' && (!terminalProvider.trim() || !paymentReference.trim())) return
    const sale: Sale = { organizationId: user!.organizationId, businessName: appName, currency, id: crypto.randomUUID(), items: cartProducts.map((product) => ({ productId: product.id, productName: product.name, quantity: cart[product.id], price: product.price })), total: cartTotal, createdAt: new Date().toISOString(), syncStatus: 'pending', paymentMethod, terminalProvider: terminalProvider.trim(), paymentReference: paymentReference.trim() }
    if (isBrowserPwa()) {
  ...
  async function scanBarcode(target: 'product' | 'payment' = 'product') {
    const accept = target === 'payment' ? acceptPaymentReference : acceptBarcode
    if (scanning) return
    ...
    try {
      ...
      while (session === cameraSession.current) {
        ...
        if (codes[0]) { accept(codes[0].rawValue); stopScan(); break }
        await new Promise(resolve => window.setTimeout(resolve, 150))
      }
    } catch {
      if (session === cameraSession.current) { stopScan(); accept(window.prompt('Camera unavailable. Enter or scan the barcode') || '') }
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {active === 'Settings' && user.role === 'admin' && <section className="panel full-panel"><PrinterSettings /><TerminalSettings key={user.organizationId} businessId={user.organizationId} defaultProvider={posProvider} onSaved={() => setTerminalRevision(value => value + 1)} /></section>}
      ...
      {active === 'Settings' && user.role === 'owner' && <section className="panel full-panel settings-panel"><div className="panel-heading"><div><h2>Business settings</h2><p>Customize the identity your team sees across the app.</p></div><Settings2 size={20} /></div><AsyncForm className="settings-form" busyLabel="Saving settings..." onSubmit={saveAppName}><label>App name<span>This appears in the sidebar and installed app.</span><input value={appName} maxLength={60} onChange={(event) => { setAppName(event.target.value); setSettingsMessage('') }} /></label><label>Currency<span>Used for product prices, wallets, sales, and receipts.</span><select value={currency} onChange={(event) => setCurrency(event.target.value)}><option value="USD">USD - US Dollar</option><option value="NGN">NGN - Nigerian Naira</option><option value="GHS">GHS - Ghanaian Cedi</option><option value="KES">KES - Kenyan Shilling</option><option value="GBP">GBP - Pound Sterling</option><option value="EUR">EUR - Euro</option></select></label><label>Default payment-terminal provider<span>Shared business default. Each checkout can override it in Payment terminal settings below.</span><input value={posProvider} placeholder="e.g. OPay" maxLength={100} onChange={(event) => setPosProvider(event.target.value)} /></label><SubmitButton className="primary-button">Save business settings <ArrowUpToLine size={17} /></SubmitButton>{settingsMessage && <p className="settings-message">{settingsMessage}</p>}</AsyncForm><PrinterSettings /><TerminalSettings key={user.organizationId} businessId={user.organizationId} defaultProvider={posProvider} onSaved={() => setTerminalRevision(value => value + 1)} />{!isBrowserPwa() && <AsyncForm className="settings-form" busyLabel="Updating password..." onSubmit={changePassword}><h3>Change password</h3><label>Current password<input name="currentPassword" type="password" placeholder="Current password" /></label><label>New password<input name="newPassword" type="password" placeholder="New password" /></label><label>Confirm password<input name="confirmPassword" type="password" placeholder="Confirm new password" /></label><SubmitButton className="primary-button" type="submit">Update password</SubmitButton>{passwordMessage && <p className="settings-message">{passwordMessage}</p>}</AsyncForm>}{isBrowserPwa() && <p className="settings-message">To reset your cloud password, log out and choose Forgot password on the sign-in screen.</p>}</section>}
    </main>
    ...
    {lastReceipt && <div className="print-receipt"><h2>{lastReceipt.businessName || appName}</h2><p>Receipt: {lastReceipt.id}</p>{(!lastReceipt.currency || !lastReceipt.businessName) && <p>Historical currency/business identity unavailable; current settings used.</p>}<p>{new Date(lastReceipt.createdAt).toLocaleString()}</p>{lastReceipt.items.map((item) => <p key={item.productId}>{item.productName || item.productId}<br />{item.quantity} Ã— {new Intl.NumberFormat(undefined, { style: 'currency', currency: lastReceipt.currency || currency }).format(item.price)}</p>)}<strong>Total: {new Intl.NumberFormat(undefined, { style: 'currency', currency: lastReceipt.currency || currency }).format(lastReceipt.total)}</strong><p>Payment: {lastReceipt.paymentMethod === 'external-pos' ? 'POS terminal' : lastReceipt.paymentMethod === 'cash' ? 'Cash' : 'Customer wallet'}</p>{lastReceipt.paymentMethod === 'external-pos' && <>{lastReceipt.terminalProvider && <p>Provider: {lastReceipt.terminalProvider}</p>}{lastReceipt.paymentReference && <p>Reference: {lastReceipt.paymentReference}</p>}</>}</div>}
  </div>
}
