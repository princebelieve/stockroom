...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {active === 'POS' && (posAccess.blocked || posAccess.status === 'grace') && <section className="panel full-panel" role="status"><h2>{posAccess.blocked ? 'POS access paused' : 'Subscription grace period'}</h2><p>{posAccess.reason}</p><a className="primary-button" href={posAccess.portalUrl || `${(import.meta.env.VITE_SYNC_API_URL || 'https://stockroom-0vm5.onrender.com').replace(/\/$/, '')}/subscriptions`} target="_blank" rel="noopener noreferrer">Manage subscription</a> <AsyncButton className="text-button" busyLabel="Checking..." onClick={async () => setPosAccess(await readPosAccess(authToken, user.organizationId, true))}>Check access again</AsyncButton></section>}
      ...
      {active === 'Settings' && user.role === 'owner' && <section className="panel full-panel"><h3>Subscription</h3><p>View your expiry date and renew manually with Paystack. Sign in with your cloud owner account. No automatic debits.</p><a className="primary-button" href={`${(import.meta.env.VITE_SYNC_API_URL || 'https://stockroom-0vm5.onrender.com').replace(/\/$/, '')}/subscriptions`} target="_blank" rel="noopener noreferrer">Manage subscription</a></section>}
