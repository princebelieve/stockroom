...
function App() {
  ...
  useEffect(() => {
    ...
    fetch('/api/customer-display', { method: 'PUT', headers: { 'Content-Type': 'application/json', ...authHeaders }, body: JSON.stringify({ businessName: appName, currency, items: displaySale.items, total: displaySale.total, completed: Boolean(completedCustomerSale) }) }).catch(() => undefined)
  }, [appName, cart, cartProducts, cartTotal, completedCustomerSale])
...
function CustomerDisplayPairing({ pairing, createPairing, openSecondMonitor }: { pairing: { url: string; code: string; expiresAt: string } | null; createPairing: () => Promise<void>; openSecondMonitor: () => Promise<void> }) {
  return <section className="panel full-panel pairing-panel"><div className="panel-heading"><div><h2>Customer display</h2><p>Share a read-only live order view with a customer or a second display on the same shop Wi-Fi.</p></div><Store size={20} /></div><ol><li>Connect the customer phone/tablet and checkout computer to the same private Wi-Fi.</li><li>Create a link and send or open it on that customer device within five minutes.</li><li>The customer can review the live basket and see the completed-sale confirmation, but cannot change anything.</li></ol><div className="report-actions">{window.stockroomDesktop && <button className="primary-button" onClick={openSecondMonitor}>Open customer display on second monitor</button>}<button className="filter-button" onClick={createPairing}>Create customer share link</button></div>{pairing && <section className="pairing-code"><span>Send or open this customer link</span><strong>{pairing.url}</strong><button className="filter-button" onClick={() => navigator.clipboard?.writeText(pairing.url).catch(() => window.prompt('Copy this customer link', pairing.url))}>Copy link</button><small>Pairing code: <b>{pairing.code}</b> · expires {new Date(pairing.expiresAt).toLocaleTimeString()}</small></section>}<p className="settings-message">After the link is opened, its read-only display session expires after 12 hours or when the checkout app restarts.</p></section>
}
