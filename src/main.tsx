...
import { AlertTriangle, ArrowDownToLine, ArrowLeft, ArrowUpToLine, BarChart3, Boxes, CheckSquare, CloudOff, Download, Eye, EyeOff, LayoutDashboard, Menu, MoreHorizontal, PackagePlus, Plus, Printer, RefreshCw, Search, ScanLine, Settings2, ShoppingCart, SlidersHorizontal, Store, UserRoundCog, WalletCards, Wifi, X } from 'lucide-react'
...
function App() {
  ...
  useEffect(() => {
    if (!showAdd) return
    const barcodeInput = document.querySelector('form.modal input[name="barcode"]') as HTMLInputElement | null
    if (!barcodeInput || barcodeInput.parentElement?.classList.contains('barcode-field')) return
    const field = document.createElement('span')
    field.className = 'barcode-field'
    barcodeInput.parentElement?.insertBefore(field, barcodeInput)
    field.append(barcodeInput)
    const scan = document.createElement('button')
    scan.type = 'button'
    scan.className = 'barcode-scan-button'
    scan.title = 'Scan product barcode'
    scan.setAttribute('aria-label', 'Scan product barcode')
    scan.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 5v4M3 15v4M21 5v4M21 15v4M7 5v14M10 5v14M14 5v14M17 5v14"/></svg>'
    scan.addEventListener('click', () => { void scanBarcode('intake', value => { barcodeInput.value = value; barcodeInput.dispatchEvent(new Event('input', { bubbles: true })) }) })
    field.append(scan)
    return () => { scan.remove() }
  }, [showAdd])
  ...
  useEffect(() => {
    ...
    if (!select) return
    for (const [value, label] of [['bank-transfer', 'Bank transfer'], ['multiple', 'Multiple payment']]) {
      if (!select.querySelector(`option[value="${value}"]`)) select.append(new Option(label, value))
    }
  ...
  const { cartProducts, cartTotal } = summarizeCart(products, cart)
  ...
  useEffect(() => { if (active === 'Wallet' || active === 'POS') void refreshWallets().catch(() => undefined) }, [active, authToken])
  ...
  if (!user) return registrationAvailable && registrationRequested
    ? registrationStage === 'request'
      ? <RegistrationRequestScreen onContinue={() => setRegistrationStage('complete')} onCancel={() => { setRegistrationRequested(false); setAuthError('') }} />
      : <SetupScreen keyRegistration onCancel={() => setRegistrationStage('request')} onCreate={registerBusiness} error={authError} setError={setAuthError} />
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {active === 'Inventory' && <section className="panel full-panel"><div className="panel-heading"><div><h2>All inventory</h2><p>Adjust counts as stock comes in or goes out.</p></div><button className="primary-button" onClick={() => setShowAdd(true)}><Plus size={18} />Add product</button></div><div className="search-row"><div className="search-box barcode-search"><Search size={17} /><input placeholder="Search products" value={query} onKeyDown={event => { if (event.key === scannerSettings().suffix) { event.preventDefault(); acceptBarcode(query) } }} onChange={(event) => setQuery(event.target.value)} /><button type="button" className="barcode-scan-button" onClick={() => scanBarcode()} aria-label="Scan product barcode" title="Scan product barcode"><ScanLine size={19} /></button></div></div><div className="table-wrap"><table><thead><tr><th>Product</th><th>SKU</th><th>Category</th><th>Stock</th><th>Unit price</th><th>Updated</th><th></th></tr></thead><tbody>{filteredProducts.map((product) => <ProductRow key={product.id} product={product} updateStock={updateStock} money={formatMoney} detailed />)}</tbody></table></div></section>}
...
function RegistrationRequestScreen({ onContinue, onCancel }: { onContinue: () => void; onCancel: () => void }) {
  ...
  const prepareRequest = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (/^[a-f0-9]{32}$/.test(referral)) sessionStorage.setItem('stockroom-referral-code', referral)
    else sessionStorage.removeItem('stockroom-referral-code')
    const message = ['Hello, I would like a Stockroom Business registration key.', `Business: ${businessName}`, `Owner: ${ownerName}`, `Email: ${email}`, ...(referral ? [`Referral code: ${referral}`] : [])].join('\n')
    const subject = 'Stockroom Business registration key request'
    const links = {
      whatsapp: `https://wa.me/2347033928277?text=${encodeURIComponent(message)}`,
      email: `mailto:info@sbi.globalcreest.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(message)}`,
    }
    setRequestLinks(links)
    window.open(links.whatsapp, '_blank', 'noopener,noreferrer')
  }
  return <main className="login-screen"><section className="login-card"><div className="brand-mark"><Boxes size={21} /></div><h1>Request a registration key</h1><p>Send your business details to our team. We will provide a key so you can finish setting up your Stockroom account.</p><form className="settings-form" onSubmit={prepareRequest}><label>Business name<input name="businessName" required maxLength={60} placeholder="My Business" /></label><label>Owner name<input name="ownerName" required maxLength={100} placeholder="Your name" /></label><label>Email<input name="email" type="email" required placeholder="you@example.com" /></label><label>Referral code (optional)<input name="referralCode" pattern="[a-f0-9]{32}" defaultValue={referralCode} /></label><button className="primary-button" type="submit">Request key on WhatsApp</button></form><p className="settings-message">WhatsApp opens with a prepared message. Review and send it there. Your details are not submitted until you send the message.</p>{requestLinks && <div className="report-actions"><a className="filter-button" href={requestLinks.whatsapp} target="_blank" rel="noreferrer">Open WhatsApp request</a><a className="filter-button" href={requestLinks.email}>Send by email</a></div>}<p><a href="https://sbi.globalcreest.com/" target="_blank" rel="noreferrer">Visit S. B. Ibhadode technology</a></p><div className="report-actions"><button type="button" className="primary-button" onClick={onContinue}>I have a registration key</button><button type="button" className="text-button" onClick={onCancel}>Back to sign in</button></div></section></main>
}

function SetupScreen({ onCreate, error, setError, keyRegistration = false, onCancel }: { keyRegistration?: boolean; onCancel?: () => void; onCreate: (event: React.FormEvent<HTMLFormElement>) => Promise<void>; error: string; setError: (value: string) => void }) {
  return <main className="login-screen"><AsyncForm busyLabel="Creating account..." className="login-card" onSubmit={async (event) => { event.preventDefault(); setError(''); try { await onCreate(event) } catch (loginError) { setError(loginError instanceof Error ? loginError.message : 'Unable to create your business account.') } }}><div className="brand-mark"><Boxes size={21} /></div><h1>Set up your shop</h1><p>{keyRegistration ? 'Enter your registration key to create the owner account and set up this device. An internet connection is required.' : 'Create the first owner account for this store.'}</p>{keyRegistration ? <label>Business registration key<input name="registrationKey" required autoComplete="off" placeholder="SBIT-?" /></label> : <label>Business name<input name="shopName" required placeholder="My Business" /></label>}<label>Owner name<input name="ownerName" required placeholder="John Doe" /></label><label>Email<input name="email" type="email" required placeholder="owner@yourshop.com" /></label><label>Password<div className="password-wrap"><input name="password" type="password" minLength={10} maxLength={256} required placeholder="Create a password" /><button type="button" className="icon-button password-toggle" aria-label="Show password" onClick={(event) => { const input = (event.currentTarget.parentElement?.querySelector('input') as HTMLInputElement | null); if (!input) return; const shouldShow = input.type === 'password'; input.type = shouldShow ? 'text' : 'password'; event.currentTarget.setAttribute('aria-label', shouldShow ? 'Hide password' : 'Show password'); }}><Eye size={16} /></button></div></label>{keyRegistration && <><label>Confirm password<input name="confirmPassword" type="password" required minLength={10} maxLength={256} autoComplete="new-password" /></label><label>Business currency<select name="currency" defaultValue="NGN">{['NGN', 'USD', 'GHS', 'ZAR', 'KES', 'XOF'].map(code => <option key={code}>{code}</option>)}</select></label><label>Referral code (optional)<input name="referralCode" pattern="[a-f0-9]{32}" defaultValue={sessionStorage.getItem('stockroom-referral-code') || ''} /></label></>}<p className="settings-message">{keyRegistration ? 'The key identifies your business and owner email. Manage your plan and payments from Subscription after signing in.' : 'Your database is configured by the developer. After payment, the owner can change their password from Admin settings.'}</p>{error && <div className="auth-error">{error}</div>}<SubmitButton className="primary-button login-button">Create owner account</SubmitButton>{onCancel && <button type="button" className="text-button" onClick={onCancel}>Back to sign in / existing business</button>}</AsyncForm></main>
}
...
function TeamManagement({ staff, loaded, addStaff, updateStaffRole, setCashierAccess, resetCashierPassword, canCreateStaff, message }: { staff: StaffUser[]; loaded: boolean; addStaff: (event: React.FormEvent<HTMLFormElement>) => Promise<void>; updateStaffRole: (id: string, role: 'admin' | 'cashier', operationalAccess?: boolean) => Promise<void>; setCashierAccess: (id: string, enabled: boolean) => Promise<void>; resetCashierPassword: (member: StaffUser) => Promise<void>; canCreateStaff: boolean; message: string }) {
  ...
  return <section className="panel full-panel team-management"><div className="panel-heading"><div><h2>Team management</h2><p>Owner controls team roles and access. Cashiers start POS-only. Admins can manage operations when enabled.</p></div><UserRoundCog size={20} /></div><div className="team-grid"><section><h3>Current team</h3><div className="table-wrap"><table><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Access</th><th>Created</th></tr></thead><tbody>{staff.length ? staff.map((member) => <tr key={member.id}><td><strong>{member.name}</strong></td><td>{member.email || '—'}</td><td>{member.role === 'owner' ? <span className={`role-badge ${member.role}`}>{member.role}</span> : <select value={member.role} onChange={(event) => { const nextRole = event.target.value as 'admin' | 'cashier'; if (nextRole !== member.role) void updateStaffRole(member.id, nextRole, nextRole === 'admin' ? true : false) }}><option value="admin">Admin</option><option value="cashier">Cashier</option></select>}</td><td>{member.role === 'cashier' ? <label><input type="checkbox" checked={Boolean(member.operationalAccess)} onChange={(event) => setCashierAccess(member.id, event.target.checked)} /> Operational</label> : member.role === 'admin' ? 'Manage operations' : 'Owner access'}</td><td>{formatCreatedDate(member.createdAt)}</td></tr>) : <tr><td colSpan={5} className="empty-state">Loading team accounts…</td></tr>}</tbody></table></div></section>{canCreateStaff && <AsyncForm className="settings-form team-form" busyLabel="Creating account..." onSubmit={addStaff}><h3>Add team member</h3><label>Full name<input name="name" required maxLength={100} placeholder="e.g. Ada Okafor" /></label><label>Email address <span>Optional contact information; it cannot control this staff account.</span><input name="email" type="email" placeholder="ada@yourbusiness.com (optional)" /></label><label>Access role<select name="role" defaultValue="cashier"><option value="cashier">Cashier — POS only by default</option><option value="admin">Admin — manage stock and operations</option></select></label><label>Temporary password<div className="password-wrap"><input name="password" type={showPassword ? 'text' : 'password'} minLength={10} required autoComplete="new-password" placeholder="At least 10 characters" /><button type="button" className="icon-button password-toggle" onClick={() => setShowPassword(current => !current)} aria-label={showPassword ? 'Hide temporary password' : 'Show temporary password'}>{showPassword ? <EyeOff size={16} /> : <Eye size={16} />}</button></div></label><label>Confirm temporary password<div className="password-wrap"><input name="passwordConfirmation" type={showConfirmation ? 'text' : 'password'} minLength={10} required autoComplete="new-password" placeholder="Re-enter temporary password" /><button type="button" className="icon-button password-toggle" onClick={() => setShowConfirmation(current => !current)} aria-label={showConfirmation ? 'Hide confirmation password' : 'Show confirmation password'}>{showConfirmation ? <EyeOff size={16} /> : <Eye size={16} />}</button></div></label><SubmitButton className="primary-button" type="submit">Create account <UserRoundCog size={17} /></SubmitButton></AsyncForm>}</div>{message && <p className="settings-message">{message}</p>}</section>
}
...
function CashierPasswordReset({ staff, resetCashierPassword }: { staff: StaffUser[]; resetCashierPassword: (member: StaffUser) => Promise<void> }) {
  ...
  return <section className="panel full-panel cashier-password-control"><div className="panel-heading"><div><h2>Cashier password control</h2><p>Cashier email addresses cannot reset accounts. Set a replacement password here and share it privately.</p></div></div><label>Cashier<select value={cashierId} onChange={(event) => setCashierId(event.target.value)}>{cashiers.map((member) => <option key={member.id} value={member.id}>{member.name} — {member.email}</option>)}</select></label><button type="button" className="primary-button" onClick={() => { const cashier = cashiers.find((member) => member.id === cashierId); if (cashier) void resetCashierPassword(cashier) }}>Reset cashier password</button></section>
}
...
function InstallerScreen({ onActivate, message, onRegister }: { onRegister?: () => void; onActivate: (event: React.FormEvent<HTMLFormElement>) => Promise<void>; message: string }) {
  ...
  if (mode === 'choose') return <main className="login-screen"><section className="login-card installer-card"><div className="brand-mark"><Boxes size={21} /></div><h1>Welcome to Stockroom</h1><p>Set up your business on this device. You only need an internet connection for this first setup.</p>{onRegister && <><h2>Starting a new business?</h2><p>Have the registration key issued to your owner email ready. Create your owner password, and Stockroom will set up this device for you.</p><button type="button" className="primary-button login-button" onClick={onRegister}>New business with a key</button></>}<h2>Already registered?</h2><p>Use your existing owner email and password to connect this device. You do not need another registration key.</p><button type="button" className="filter-button" onClick={() => setMode('existing')}>Existing business</button><p><a href="https://stockroom.globalcreest.com/welcome.html" target="_blank" rel="noreferrer">About Stockroom and downloads</a></p>{!isNativeMobile() && !isBrowserPwa() && <details><summary>Developer installation tools</summary><p>For the developer's existing manual installation process only.</p><button type="button" className="text-button" onClick={() => setMode('new')}>Installer activation</button></details>}{message && <p role="status">{message}</p>}</section></main>
  return <main className="login-screen"><AsyncForm className="login-card installer-card" busyLabel="Connecting device..." onSubmit={onActivate}><input type="hidden" name="mode" value={mode} /><div className="brand-mark"><Boxes size={21} /></div><h1>{mode === 'new' ? 'Developer installation' : 'Add another device'}</h1><p>{mode === 'new' ? 'Use the existing installer credentials to activate this device for a client.' : 'Enter the owner credentials you created when registering your business. Stockroom will download your business data.'}</p>{mode === 'new' && <label>Client business ID<input name="businessId" required pattern="[a-z0-9][a-z0-9-]{2,80}" /></label>}<label>Device label<input name="label" required maxLength={100} placeholder="Main checkout computer" /></label>{mode === 'new' ? <label>Installer Admin API key<input name="adminApiKey" type="password" required autoComplete="off" /></label> : <><label>Owner email<input name="ownerEmail" type="email" required autoComplete="username" /></label><label>Owner password<input name="ownerPassword" type="password" minLength={10} required autoComplete="current-password" /></label></>}{message && <p role="status">{message}</p>}<SubmitButton className="primary-button login-button">{mode === 'new' ? 'Activate new client' : 'Connect this device'}</SubmitButton><button type="button" className="text-button" onClick={() => setMode('choose')}>Back to setup choices</button></AsyncForm></main>
}
