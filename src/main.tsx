...
function App() {
  ...
  async function completeSale() {
    ...
    let sale: Sale
    try {
      sale = recordPayment({ organizationId: user!.organizationId, businessName: appName, currency, id: crypto.randomUUID(), items: cartProducts.map((product) => ({ productId: product.id, productName: product.name, quantity: cart[product.id], price: product.price })), total: cartTotal, createdAt: new Date().toISOString(), syncStatus: 'pending', paymentMethod, terminalProvider: terminalProvider.trim(), paymentReference: paymentReference.trim(), paymentDetails: paymentMethod === 'wallet' ? {} : { amountReceived: paymentMethod === 'cash' ? cashReceived : cartTotal + Number(extraKept || 0), extraKept, reason: extraReason, note: extraNote } }, extraPaymentPolicy) as Sale
