...
test('cash sales ignore retained extras and always calculate the change due', () => {
  ...
  assert.equal(sale.paymentDetails.extraKept, 0)
  assert.equal(sale.paymentDetails.changeGiven, 5)
  ...
  assert.equal(recordPayment({ paymentMethod: 'cash', total: 200, paymentDetails: { amountReceived: '1000', extraKept: '0' } }, policy).changeGiven, 800)
})
